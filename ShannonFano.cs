...
namespace ShannonFano
{
    public partial class ShannonFano : Form
    {
        ...
        void Opening()
        {
            ...
            long length = new System.IO.FileInfo(openFileDialog.FileName).Length; //вычисление размера стартового файла
            startsize.Text = Convert.ToString(length) + " Б";        
        ...
        void Coding()
        {
            ...
            try
            {
                ...
                double length = new System.IO.FileInfo(openFileDialog.FileName).Length;

                double endlength = new System.IO.FileInfo(openFileDialog.FileName + ".bin").Length;
                endfilesize.Text = Convert.ToString(endlength) + " Б";


                double proc = ((endlength - length) / length);
                double procen = Math.Round(proc, 0);
                procent.Text = Convert.ToString(procen) + "%";
